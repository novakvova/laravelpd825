                            ...
                            <div class="table-responsive">
                                <table class="table table-striped">
                                    ...
                                    @foreach($items as $item)
                                        <tr id="{{$item->id}}">
                                            <td><img src="/images/105_{{$item->productImages[0]->name}}" width="75">
                                            </td>
                                            <td>{{$item->name}}</td>
                                            @if($item->count>=1)
                                                <td>In stock</td>
                                            ...
                                            @else
                                                <td>Not in stock</td>
                                                <td style="width: 100px"><input class="form-control" disabled
                                                                                type="number"
                                                                                min="1"
                                                                                max="2" value="0"/></td>
                                            @endif
                                            <td class="text-right prc">{{$item->price}}</td>
                                            <td class="text-right">
                                ...
                                <div class="col-sm-12 col-md-6 text-right">
                                    <form id="create" method="post" action="{{ route('cart.store') }}">
                                        ...
                                        @if($items->contains('count',0))
                                            <button data-toggle="tooltip" data-placement="top" disabled
                                                    title="One of products not in stock"
                                                    class="btn btn-lg btn-block btn-secondary text-uppercase">Make order
                                            </button>
                                        @else
            ...
            $(document).ready(function () {
                function CalcTotal() {
                    ...
                    trs.each(function () {
                        ...
                        let price = $(this).find(".prc")[0].innerText;
                        glPrice += (parseInt(count) * parseInt(price));
                    });
                    console.log(total);
